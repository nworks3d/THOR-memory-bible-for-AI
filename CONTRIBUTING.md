...
This repository ships no eval corpus and no measurement harnesses. That is
deliberate. A corpus authored alongside the mechanism it validates measures the
author, not the tool, and publishing that score invites everyone else to
re-run someone else's exam instead of testing their own store. If a change needs
evidence, build the corpus for it, freeze it before you write the mechanism,
keep a holdout, and put the numbers in the pull request.
...
Releases are cut from a tag by CI, never from a maintainer's machine. They are
not on a schedule: THOR ships when there is something worth shipping.
