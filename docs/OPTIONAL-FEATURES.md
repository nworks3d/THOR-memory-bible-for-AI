...
> **Field-tested (2026-07):** six full real-work sessions, honestly reviewed
> end-to-end with per-hit judging, keep confirming the same short list. The
> **pins + session-start brief** were the most consistently useful surface.
> The **moment-of-action guard advisories** made the decisive catches -
> deploy and diagnosis know-how at exactly the right file or command,
> including a gotcha stored at noon that prevented a repeat regression the
> same evening. The **Stop response guard** was the only surface that changed
> agent behavior in the moment (passively injected rules were read and still
> ignored). **Per-prompt recall** earned its keep wherever a stored precedent
> existed. And **project scoping** is what keeps recall clean: in a jury
> measurement over 100 real prompts, moving domain facts out of the global
> tier eliminated the cross-domain noise class entirely. That is n=6 sessions
> on one setup - strong field evidence, not a controlled benchmark.
...
| I want to try THOR before wiring anything in | Nothing. Write with `thor create <id> "<body>"` and read with `thor recall`, both by hand | All of it, until you like it |
...
- [Hooks: automatic recall, guards and nudges](#hooks-automatic-recall-guards-and-nudges)  
  covers: thor install; thor install --with-courier; thor session-start; thor install --with-daemon; thor install --with-guard; thor install --settings &lt;path&gt;; thor install --backup-repo &lt;path&gt;; Registering THOR as an MCP server (thor mcp); Running THOR with no hooks at all
...
- [Keeping the memory healthy](#keeping-the-memory-healthy)  
  covers: thor doctor; thor fsck; thor fsck --rebuild-fts; thor symbols; thor consolidate; thor consolidate --apply-dedup; thor consolidate --min-age-events &lt;N&gt;; thor steward; thor pin &lt;id&gt; / thor unpin &lt;id&gt;; thor mark &lt;id&gt; (and --noise); expires: YYYY-MM-DD (on the MCP remember tool)
...
- [Running THOR as a remote server](#running-thor-as-a-remote-server)  
  covers: thor mcp --http &lt;bind&gt;; The Docker deployment (deploy/Dockerfile and deploy/docker-compose.yml); THOR_RECV_BIND (container as the replica); THOR_REPLICA_URL (container as the authority)
- [Switching things off](#switching-things-off)  
  covers: THOR-SILENT.flag; THOR-PRIMARY.flag; THOR-EXP-HIST-DEMOTE.flag; THOR-EXP-AUTO-ECHO.flag; THOR-NO-PIN-DEDUP.flag; THOR-NO-SCOPE-HINT.flag; THOR-NO-WEAK-NOTE.flag
- [Coming from mimir](#coming-from-mimir)  
  covers: python thor/tools/export_mimir.py
...
- **What it costs:**
  ...
  - The program itself grows from about 10 MB to about 35 MB (SETUP.md
...
- **Leave it off if:** you are on Windows or Linux x86_64 and one of the three assets
  fits. Release assets are built by CI from the tag on
...
- **How to turn it on:** copy the template into the per-user folder, then edit it. Every string in it is an example and must be replaced. From the repo root:
  ...
  - `min_chars` is an optional length floor: the rule stays quiet unless the searched text is at least that many characters long. It exists for the *response* rulebook below, where the text is your assistant's whole message. A rule about the shape of a report ("open with a plain-language summary") should not fire on a two-line status note that happens to contain one technical word - and no word list can tell those two apart, while a length can. Leave it out and there is no floor, exactly as before.
  ...
  - `id` is read but never printed on the command side. On the response side it is worth writing carefully: `thor consolidate` reads your reminders, and a reminder that cites the memory id behind the rule ("your standing rule (mem-1234abcd): ...") lets it check whether that fact carries the `guarded` tag. See the consolidate section.
...
- **How to turn it on:** copy the template and rewrite the phrases to match how your agent actually talks to you. The five shipped rules are the ones that earned their place in daily use, written generically so you can start from them rather than from an empty file: asking permission for something you already asked for, claiming it cannot do a thing without trying, asking you to fetch what it could read itself, opening a wall of results with no plain-language summary, and handing back a patch instead of the finished artifact. Each carries a short `none_of` list so the rule stays quiet when you are *talking about* the rule instead of breaking it - keep that habit when you add your own, or discussing your rulebook will trip it.
...
It also prints your **anchor coverage** - how many live facts carry a guard
anchor, and so can reach the moment-of-action channel at all - followed by the
typed facts that name a file or command in their body but have no anchor yet,
proven-useful first, each with the concrete anchor to consider. That list is a
...
your reminders for a memory id, so a reminder that cites the fact it enforces
("your standing rule (mem-1234abcd): ...") is checked for you. No rulebook, or
reminders that cite nothing, simply means this section is always empty.
...
- **MCP niceties (the tool, not the CLI):** the MCP `mark` rejects unknown
  ...
  truncated id that matches exactly one live entity resolves automatically (the
  reply says `resolved from '...'`; ambiguity never resolves). And a repeat
  noise mark on a GLOBAL fact comes back with the structural hint: that is
  usually a scope problem - the fix is `reproject`, not more ranking.
- **How to turn it on:**
...
- **The guard checks the date too.** Recall filters expired facts at read time,
  and the guard's moment-of-action pass - the one that surfaces facts anchored
  to an exact file path or command - does the same strict check before it
  fires (expired means `expires` is before today, the same comparison on both
  surfaces). **This changed:** before 2026-07-26 the guard's anchored pass
  skipped this check entirely, so an expired fact with an anchor kept firing
  at the moment of action forever, even after recall had already stopped
  serving it. `thor get`/`thor history` are unaffected either way - they
  always show an expired fact in full.
- **A warning when this looks like a rule, not a report.** THOR cannot judge
  whether a fact deserves to live forever, but it can read the opening line:
  a body that opens with a rule marker (`HARDE REGEL`, `HARD RULE`, `GOTCHA`,
  `REGEL:`, `VOORKEUR`, `PREFERENCE`, `DOCTRINE`) and also carries an
  `expires` date - set through `remember` or `revise` - is still stored or
  revised as asked, but the reply adds a warning that reports expire, rules
  never, and this one is about to silently vanish on its date. It is a
  warning, not a refusal: a genuinely temporary rule ("pin to v1.9 until the
  upstream fix lands") legitimately carries a date, so THOR asks rather than
  blocks. The doctrine behind this and the guard note above is one line:
  **reports expire, RULES never.** A rule buried inside an expiring report (a
  status update, a milestone write-up) belongs lifted into its own
  never-expiring fact before the report is allowed to expire - `thor
  steward`'s review rubric spells this out as its own numbered step (see
  below).
- **What it costs:** no extra process, no port, no download, no change in binary
...
- **How to turn it off again:** on the MCP `revise` tool this is now one call:
  ...
  `triggers`, `anchors`) - each one changes only the field you
...
- **How to check it worked:** a one-shot run prints
  ...
  ```sh
  thor create batch-check-1 "batch check one"
  thor create batch-check-2 "batch check two"
  thor create batch-check-3 "batch check three"
...
### THOR-EXP-HIST-DEMOTE.flag

An empty file next to the store that changes one thing in the automatic
per-prompt injection: a memory whose own opening loudly declares it a
historical record loses its injection slot to a live fact of at least equal
match strength. Demotion, not exclusion - the fact stays in the pool, still
serves when nothing live matches as strongly, and the deliberate surfaces
(`recall`, `get`) are untouched. Detection is a narrow, deliberate convention:
an UPPERCASE `HISTORI` or `ACHTERHAALD` (superseded) marker within the first
250 characters of the body. Completed-work words ("done", "resolved") are not
markers on purpose - a solved gotcha usually still carries the live lesson.

- **Default:** absent = off, byte-identical injection behavior.
- **Turn it on if:** your store keeps long milestone/archive write-ups and you
  see them crowd injection slots during unrelated work - on the author's store
  this was a recurring noise class in end-of-session reviews.
- **Leave it off if:** your archive facts carry no self-declared marker; the
  demotion then never fires and the flag is dead weight.
- **What it costs:** nothing measurable - one partition pass over at most eight
  pool hits, only while the flag exists. Measured against a 59-scenario live
  drift corpus: zero regression (a historical fact that is itself the strongest
  match keeps its slot by design).
- **How to turn it on:** create the empty file next to `thor.db`.
- **How to check it worked:** a query whose top matches include a marked
  archive fact and a comparably-strong live fact now serves the live fact
  first.
- **How to turn it off again:** delete the file - the hooks re-read flags on
  every call, so it takes effect on the next prompt.

### THOR-EXP-AUTO-ECHO.flag

An empty file next to the store that mechanically settles part of the "did
this injected fact actually help?" question. While it exists, the
before-a-tool-runs guard records what the session touches (file paths, command
lines) in the local ledger, and at every Stop THOR compares each memory the
courier served against that touched list. On strong evidence only - a declared
anchor was touched, or at least three distinct fires-when words (or one
identifier-shaped one, 8+ characters with `.`/`/`/`_`/`-`) appear in the
touched set - it appends a `fact_echoed` event labeled `auto-echo:` with the
evidence spelled out. Auto echoes count at half the weight of a manual `mark`,
and they count ONLY while the flag exists: deleting the flag makes every auto
event in the log retroactively inert. Noise judgments are never automated.

- **Default:** absent = off. No per-tool-call ledger writes, no Stop-time
  settlement, and any auto events already in the log count for nothing.
- **Turn it on if:** you want the usage-strength signal fed without relying on
  agent discipline - measured before this existed, the `mark` tool had been
  called three times against 579 recalls. Early real-session numbers: every
  hand-checked anchor-based echo was correct; word-based echoes are tuned
  precision-first and deliberately low-recall.
- **Leave it off if:** you do not use `mark`/usage strength at all, or the
  session runs against a replica (the settlement skips replicas by design - a
  direct append there would fork the shipped log).
- **What it costs:** one small ledger write per tool call that carries a file
  path or command, one store open per Stop, and permanent `fact_echoed` events
  in the hash-chained log. They travel with log shipping, decay by the same
  2000-event half-life as manual marks, and are inert whenever the flag is
  absent.
- **How to turn it on:** create the empty file next to `thor.db`.
- **How to check it worked:** after a session that acted on a served fact,
  `thor history <id>` shows a `fact_echoed` event whose body starts with
  `auto-echo:` and names the matched anchor or words.
- **How to turn it off again:** delete the flag. New echoes stop immediately
  and the existing auto events stop counting.

### THOR-NO-PIN-DEDUP.flag

An empty file next to the store that puts back a behaviour THOR dropped on
purpose. A pinned fact is already delivered in full twice over: the
`<thor-brief>` re-injects the whole pin list at every session start and right
after each compaction. Since that is verbatim in the agent's context, the
per-prompt recall block stops spending one of its three slots on it - pinned
facts move to the back of the working pool, so they still surface when nothing
else matched, and a deeper fact rotates into the freed slot instead.

The same flag covers a second case, added 2026-07-26: a fact you tag `guarded`.
That tag is your statement that this fact already has a gate of its own - guard
anchors naming the exact command or file it governs (so it is injected the
moment you touch them), or a rule in the response rulebook that holds the answer
back until the fact is honoured. A gate that fires at the moment of action is
strictly stronger than a reminder repeated in every prompt, so the reminder
steps aside. It is deliberately something you tag by hand: only the person who
wrote the gate knows it exists, and a wrong tag costs a demotion, never a
deletion.

- **Default:** absent = the deduplication is ON (that is the shipped
  behaviour). Create the file to go back to serving pins and `guarded` facts in
  the slots too.
- **Turn it on (i.e. switch dedup OFF) if:** you deliberately want your standing
  rules repeated close to every prompt rather than once at session start - for
  instance because you run very long sessions and value the repetition over the
  slot.
- **Leave it absent if:** you want the recall block to carry things you have not
  already been told. Measured on 100 real prompts: 13.9% of all served slots
  were going to a pinned fact, and 37% of prompts carried at least one such
  duplicate. Switching the dedup on cut that by 93% with the drift-catch
  measurement byte-identical - it removes repetition, not content. On a second
  100-prompt jury (2026-07-26) three of the four facts most often judged
  off-topic were gated ones, which is the case the `guarded` tag covers.
- **What it costs:** nothing to run (one read of the local pin list, which the
  block already needs, plus a tag check on hits you already have). The honest
  limit: this is hygiene, not a measured quality gain - no benchmark number
  moved in either direction.
- **How to turn it on:** create the empty file next to `thor.db`.
- **How to check it worked:** ask something your pinned rules answer. With the
  flag present the pinned fact appears as a `- [global] <id>` hit line again;
  without it, it does not. Same test with a `guarded`-tagged fact.
- **How to turn it off again:** delete the flag. Or, for one fact only, remove
  its `guarded` tag - the flag is all-or-nothing, the tag is per fact.

### THOR-NO-SCOPE-HINT.flag

An empty file next to the store that switches off the scope pointers. A pointer
is an ordinary fact you tagged `wegwijzer` (Dutch for signpost), whose whole job
is to say where a body of knowledge lives: "all finance facts live in project
Investments - recall there". THOR looks those up by their tag on every prompt,
independently of ranking, and appends at most two as a single `Scope hint:` line
below the hits. They never occupy one of the three content slots.

Why it exists: once you move domain knowledge out of the global tier into
project scopes - the right fix for cross-project noise - a chat that is not in
that project can no longer reach it. Measured on 20 blind chat prompts, only 30%
were told which scope held the answer. Pointers were already stored, but a
pointer holds no domain content, so it can never pass the relevance gate on its
own merits, and three quarters of the time it never even reached the working
pool. With the tag lookup that number is 96% on a 60-prompt blind holdout
written by three different models, while on 100 real in-project work prompts a
stray hint line appeared 3 times.

- **Default:** absent = pointers are served (the shipped behaviour).
- **Turn it on (i.e. switch pointers OFF) if:** you keep no `wegwijzer`-tagged
  facts at all, or you never work outside your project directories - then the
  lookup buys you nothing.
- **Leave it absent if:** you scope domain knowledge per project and also ask
  about it from general chats. Note the bar differs by session: with no project
  of its own, one matched keyword earns the line (that session cannot reach the
  scope any other way); inside a project two are required, because there one
  loose keyword was measured as real noise.
- **What it costs:** one extra rare-token lookup per prompt (the tag appears in
  pointer footers and nowhere in ordinary prose), measured within run-to-run
  noise, plus at most two lines of text. With the flag present, pointers go back
  to competing as ordinary facts for the ranked slots.
- **How to turn it on:** create the empty file next to `thor.db`.
- **How to check it worked:** ask, from a directory with no `.thor` marker,
  something only a scoped fact answers. Without the flag the block ends in a
  `Scope hint:` line naming the project; with it, nothing points there.
- **How to turn it off again:** delete the flag.

### THOR-NO-WEAK-NOTE.flag

An empty file next to the store that removes one line from the `recall` tool's
answer. Deliberate recall fills every slot it was asked for, so a question about
a domain your store knows nothing about still comes back with five
confident-looking lines - unrelated facts that happen to share a word. When
EVERY hit in an answer is a weak match (no strict all-terms match, no declared
trigger for the question, and not even half the question's content terms
covered), THOR appends one sentence saying so, and suggests the scope hint,
`all_projects`, or asking you instead.

It warns; it never removes. That is a measured choice, not caution: dropping
those hits instead would have suppressed 15 of 187 known answers on a
460-question battery, because a code chunk that IS the answer ("which version of
this package is pinned") shares almost no words with the question. Weak and
useless are not the same thing. As a warning the same rule costs one line: it
fires on 1.3% of that battery's questions, and on only 0.5% of the ones whose
answer was actually there.

- **Default:** absent = the note is shown when every hit is weak.
- **Turn it on (i.e. remove the note) if:** you read every recall answer
  yourself and would rather have zero extra prose in it.
- **Leave it absent if:** an agent acts on recall answers without you reading
  them first - that is exactly where a confident-looking wall of unrelated facts
  does damage, and where "THOR may hold nothing on this here" changes what the
  agent does next.
- **What it costs:** one line, on the small fraction of questions where nothing
  fit. The hits themselves are untouched in every case.
- **How to turn it on:** create the empty file next to `thor.db`.
- **How to check it worked:** ask `recall` something your store cannot possibly
  know ("how do I fix a dishwasher that will not drain"). Without the flag the
  answer ends in a `Weak match:` sentence; with it, the same hits arrive bare.
- **How to turn it off again:** delete the flag.
...
A migration helper, and the only script left in `thor/tools/`. It exports the
contents of a [mimir](https://github.com/MakerViking/mimir) SQLite database to a
JSONL snapshot that `thor import` can read, so you can carry years of memory
across instead of starting empty.

- **Default:** never runs. It is a one-time migration you invoke by hand.
- **Turn it on if:** you already use mimir and want its facts in THOR.
- **Do NOT use it if:** your THOR store already holds real work. `thor import`
  refuses to run against a store that was ever seeded, precisely so a second
  import cannot resurrect retracted facts or wipe metadata you added on the THOR
  side. Respect that refusal rather than working around it: if you need one fact
  out of an old store, retype it with `remember`.
- **What it costs:** nothing to your store until you run `thor import`. The
  export itself only reads mimir's database.
- **How to turn it on:**

```sh
python thor/tools/export_mimir.py --db <path/to/mimir.db> --out snapshot.jsonl
