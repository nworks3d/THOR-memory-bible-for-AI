...
pub fn covers_query(body: &str, query: &str) -> bool {
    let (covered, qterms) = coverage_stats(body, query);
    if qterms == 0 {
        return false;
    }
    covered >= 2.min(qterms)
}

/// The raw numbers behind [`covers_query`]: (distinct query content terms found
/// in the body, total distinct query content terms). Public so measurement
/// a harness of your own reports exactly the production counting.
pub fn coverage_stats(body: &str, query: &str) -> (usize, usize) {
    ...
    let body_tokens: HashSet<String> = body
        ...
        .map(|t| fold_diacritics(&t.to_lowercase()))
        .collect();
    let covered = qterms.iter().filter(|t| body_tokens.contains(*t)).count();
    (covered, qterms.len())
}

/// Self-declared historical fact: a body whose OPENING declares itself a
/// historical record ("HISTORISCH (2026-06-30, ...)", "HISTORIE - NOOIT GEBOUWD",
/// "[HISTORISCHE MOMENTOPNAME]", "STATUS ...: ACHTERHAALD"). Convention
/// detection, deliberately narrow and UPPERCASE-only - the store's own
/// convention shouts these labels, while lowercase "historie" in prose is just
/// a word. AFGEROND and OPGELOST are NOT markers: a completed measurement or a
/// solved gotcha often still carries the live lesson (measured 2026-07-24: the
/// useful strength fact opens with "AFGEROND"). Window: the opening of the
/// footer-stripped body - the convention puts the label up front, and a marker
/// word merely QUOTED deep inside a live fact must not relabel it.
pub fn is_historical(body: &str) -> bool {
    // "HISTORI" covers HISTORISCH / HISTORISCHE / HISTORIE in one match.
    const MARKERS: [&str; 2] = ["HISTORI", "ACHTERHAALD"];
    let head: String = crate::footer::strip(body).chars().take(250).collect();
...
pub fn trigger_authorizes(body: &str, query: &str) -> bool {
    trigger_rescues(body, query, &query_trigger_terms(query))
}

/// The query's deduped, folded content terms - the shape `trigger_evidence`
/// expects. Shared by the gate above and the weaker any-evidence door below so
/// both read the query exactly the same way.
fn query_trigger_terms(query: &str) -> Vec<String> {
    let mut seen = HashSet::new();
...
/// Is this hit only a WEAK match for the query - no strict-AND evidence, no
/// declared trigger, and not even half the question's content terms covered?
///
/// Deliberately NOT a gate. Measured on the 460-question battery (2026-07-25):
/// dropping weak hits would have suppressed 15 of 187 known answers (8%) to
/// remove 13% of served lines - a third independent rejection of statistical
/// gating on this store, after the coverage gate and the cross-encoder. The
/// reason is structural: a code chunk that IS the answer ("what version of
/// unpipe is pinned in the lockfile") shares almost no vocabulary with the
/// question, so weakness and uselessness are not the same thing here.
///
/// What it IS for: telling the agent that EVERY hit in an answer is weak, which
/// on the same battery happens for 1.3% of questions and only 0.5% of the ones
/// whose answer was actually present. An advisory costs a line when wrong; a
/// gate costs the answer.
pub fn is_weak_match(body: &str, query: &str, matched_and: bool) -> bool {
    if matched_and || trigger_authorizes(body, query) {
        return false;
    }
    let (covered, qterms) = coverage_stats(body, query);
    !(covers_query(body, query) && qterms > 0 && covered * 2 >= qterms)
}

/// Declared-trigger evidence for a SCOPE POINTER ("finance facts live in project
/// Investments"), which can never pass the coverage gate because it holds
/// pointers instead of content - measured reachability from a general chat was
/// 30% without this door.
///
/// `need` is the bar, and it is deliberately different per session, because the
/// problem is not: a chat with NO project of its own is the case that needs the
/// pointer, and there one matched term is worth a line. Inside a project the
/// session already has a scope, and one loose term was measured as real noise
/// (a Sensor-Board task mentioning "sensor" pulled the high-temp-printer
/// pointer: 9 stray lines over 58 benchmark prompts). Two terms there.
///
/// Never use this to authorize a CONTENT fact - one generic word lifting a fact
/// past the floor is the noise `trigger_rescues` exists to prevent.
pub fn trigger_matches_scope(body: &str, query: &str, need: usize) -> bool {
    let (generic, identifier, _) = trigger_evidence(body, query, &query_trigger_terms(query));
    generic + identifier >= need.max(1)
}
...
/// a recall battery over the real store (highest recall@5, no category regression).
#[cfg(feature = "semantic")]
...
mod tests {
    ...
    #[test]
    fn is_historical_matches_the_shouted_convention_only() {
        assert!(is_historical("HISTORISCH (2026-06-30; ACHTERHAALD sinds 2026-07-10): destijds..."));
        assert!(is_historical(
            "MIJLPAAL (2026-06-30, eerste capstone; HISTORISCH VERSLAG van die dag, NIET de stand van nu)"
        ));
        assert!(is_historical("MUNIN BOUW-VOORTGANG [HISTORISCHE MOMENTOPNAME - MUNIN heet sinds..."));
        assert!(is_historical("HISTORIE - NOOIT GEBOUWD, GEEN OPENSTAANDE ACTIE. PLAN-LOCK (2026-07-07)"));
        // Completed work is NOT historical: the lesson usually lives on.
        assert!(!is_historical("AFGEROND 2026-07-21: het strength-signaal wordt niet gevoed - gemeten"));
        assert!(!is_historical("OPGELOST IN DE CODE (2026-07-15, commit db500f0) - revise wist niet langer"));
        // Lowercase prose is a word, not a label.
        assert!(!is_historical("de historie van dit project is lang en achterhaald voelt het nooit"));
        // A marker deep inside a live fact must not relabel the whole fact.
        let deep = format!("{} HISTORISCH", "levende regel ".repeat(30));
        assert!(!is_historical(&deep));
    }
