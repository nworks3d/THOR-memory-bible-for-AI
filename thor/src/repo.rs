...
/// Does this PROSE claim to be about project `key`? The one owner of that
/// question, because two places ask it in opposite directions and a private
/// answer in each is how they drift apart: the courier skips a scope pointer
/// that already names the session's own project, and `consolidate` reports a
/// project that no pointer names at all.
///
/// Two rules, both from measured misfires of the plain `body.contains(key)` it
/// replaces:
///  - `[[...]]` CITATIONS DO NOT COUNT. A registry fact that links a sibling
///    fact by id (`[[The-AI-memory-bible:mem-8f41...]]`) mentions that project
///    key only as part of a link. Counting it made `consolidate` report zero
///    unpointed scopes while the busiest project in the store (60+ live facts)
///    had no pointer at all - the check answered about its own blind spot.
///  - THE KEY MUST STAND ALONE. A bare substring lets a pointer for
///    `ProjAlpha` silently cover `ProjA`. Alphanumerics and `_` next to the
///    match disqualify it; `-`, `:`, `/` and whitespace are ordinary
///    neighbours, since keys themselves contain hyphens and appear as
///    `<key>:mem-...`.
pub fn prose_mentions_project(body: &str, key: &str) -> bool {
    if key.is_empty() {
        return false;
    }
    // Drop every [[...]] span first; an unterminated one drops the rest, which
    // is the conservative direction (a citation never earns a mention).
    ...
    let boundary_ok = |c: Option<char>| !c.is_some_and(|c| c.is_alphanumeric() || c == '_');
    let mut from = 0;
    while let Some(at) = prose[from..].find(key) {
        let start = from + at;
        let end = start + key.len();
        let before = prose[..start].chars().next_back();
        let after = prose[end..].chars().next();
...
#[cfg(test)]
mod prose_mention_tests {
    use super::prose_mentions_project;

    #[test]
    fn a_citation_is_not_a_scope_claim_and_a_key_must_stand_alone() {
        // The real misfire: a registry pointer citing a fact id excused the whole
        // project from the missing-pointer check.
        assert!(!prose_mentions_project(
            "the registry, see [[The-AI-memory-bible:mem-8f41]] for the example",
            "The-AI-memory-bible"
        ));
        // A genuine claim in prose counts, wherever it sits.
        assert!(prose_mentions_project(
            "all finance facts live in project Investments - recall there",
            "Investments"
        ));
        // `<key>:mem-...` outside a citation is a claim about that scope.
        assert!(prose_mentions_project("moved to ProjA:mem-1 last week", "ProjA"));
        // A longer key must not be covered by a shorter one's pointer.
        assert!(!prose_mentions_project("everything about ProjAlpha lives here", "ProjA"));
        // Hyphens are ordinary neighbours - keys contain them.
        assert!(prose_mentions_project("see the Sensor-Board-map on the NAS", "Sensor-Board"));
        // An unterminated citation drops the tail: conservative, never a mention.
        assert!(!prose_mentions_project("dangling [[ProjA:mem-2", "ProjA"));
    }
}
