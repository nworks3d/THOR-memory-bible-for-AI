...
impl Reranker {
    ...
    pub fn rank(&mut self, query: &str, docs: &[String]) -> Result<Vec<usize>> {
        Ok(self.rank_scored(query, docs)?.into_iter().map(|(i, _)| i).collect())
    }

    /// As [`Self::rank`], but keeps the cross-encoder's raw relevance score per
    /// document (best first). Scores are comparable within one call, not across
    /// models. Measurement surface only - production ordering
    /// only ever consumes the indices.
    pub fn rank_scored(&mut self, query: &str, docs: &[String]) -> Result<Vec<(usize, f32)>> {
