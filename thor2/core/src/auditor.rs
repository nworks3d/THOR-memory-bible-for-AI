...
mod tests {
    ...
    #[test]
    fn test_item_marked_useful_only_entity_is_consistent() {
        // Same defect class as test_reprojected_only_entity_is_consistent,
        // for the new decay-marking kind: a mark recorded against an id with
        // no other events (e.g. a stale reference from an earlier session -
        // see serve::mark's own "marking an unknown id never panics" test)
        // is head-neutral, so BOTH derivations must omit the entity entirely,
        // never disagree about an empty-vs-absent head-set.
        let mut store = EventStore::in_memory().unwrap();
        store
            .append_event("s", "l", "a", EventKind::FactCreated, "real", None, "a fact")
            .unwrap();
        store
            .append_event(
                "s", "l", "a", EventKind::ItemMarkedUseful, "ghost", None,
                r#"{"marked_at":"2026-08-02T00:00:00Z"}"#,
            )
            .unwrap();
        let events = store.get_all_events().unwrap();
        assert!(
            DifferentialAuditor::verify_consistency(&events).is_ok(),
            "an item-marked-useful-only entity must not trip the differential auditor"
        );
    }
