    ...
    for event in events {
        match event.kind {
            ...
            EventKind::ItemServed | EventKind::ItemMarkedUseful | EventKind::ItemMarkedNoise => {
                ...
                // id cannot mint a phantom project entry.
            }
            _ => {
...
pub fn compute_head_sets(events: &[Event]) -> HashMap<String, HeadSet> {
    ...
    for event in events {
        match event.kind {
            ...
            // Reassert/echo/reproject/item_served/item_marked_useful never
            // change the head-set. FactReprojected is read only by the
            // project fold (compute_projects).
